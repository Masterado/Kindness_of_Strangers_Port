...
function motion(mx=true, my=true, mm=true, spx, spy){
	...
	if(my){
		...
		if(!(moty(self,spy,false))){_speed[1]=0;acceleration[1]=[0];}
...
function moty(e, _spdy, trig03){
	...
	while(i <= spdy){
		...
		if(! (place_meeting(x,y+f/**f*/,objMaze) or trig03 )     /*objMaze.coll(e.x,e.y + spdy*f,1)*/){
			//if(objMaze.coll()){}
